...
**当前状态（第 9 天 · 结课）**：主路径已端到端贯通，三脚本（preprocess / infer / evaluate）均为真实实现，真实指标 **按键准确率 88.0% / 鼠标相关系数 0.756**（阈值 55% / 0.5，评测 200 帧）；第 8 天回归 smoke 10/10 全过（4 主路径 + 4 异常 + 2 扩展），立项扩展微调四波未达 +8pp/+0.08、已归档为科学发现。第 9 天为结课交付：源码.zip、结课报告（报告09）、演示 PPT + 讲解稿 + 录屏演示（其中 PPT / 讲解稿 / 录屏 / 源码 zip 均随报告另行提交，不在本仓库；仓库内为代码 + 复现文档 + 各天报告）。
...
| `202500502046-陶子萱-实践报告01~09-…/` | 按天的实践报告与阶段文档（需求、选型、约定、报告正文） |
